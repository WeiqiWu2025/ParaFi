...
preambleForEstChannel_addTags = survey_ParaFi_funcGeneratePreamble(seqLenForEstChannel,numTags+numAddTags);

maxNumPackets = 10000; % The maximum number of packets at an SNR point

S = numel(snr); 
numBitErrs = zeros(S,numTags+numAddTags); % numBitErrs: The number of bit errors
...
%     disp(['SNR: ',num2str(snr(i)),' dB...']);
    ...
    while n<=maxNumPackets
        ...
        for tag_idx2 = numTags+1:(numTags+numAddTags)
            ...
            numTail1 = numTagData - seqLenForEstChannel*(numTags+numAddTags)-length(tmp_payload1);
            tail1 = ones(numTail1,1);
            tagData(:,tag_idx2) = [preambleForEstChannel_addTags(:,tag_idx2);tmp_payload1;tail1];
        end
        
        % perform backscatter operation
        for tag_idx3 = 1:(numTags+numAddTags)
            bxSig{tag_idx3} = survey_ParaFi_funcBackscatter(exSig(:,tag_idx3),tagData(:,tag_idx3),1);
        end
        
        %%%%%** multi-antennas Tags-RX backscatter channel & AWGN **%%%%%
        for chan_tag_rx_idx1 = 1:numMultiAntennas
            for chan_tag_rx_idx2 = 1:(numTags+numAddTags)
                reset(tgnChannel);
                rxSig{chan_tag_rx_idx1,chan_tag_rx_idx2} = tgnChannel(bxSig{chan_tag_rx_idx2});
            end
        end
        ...
        for rx_idx1 = 1:numMultiAntennas
            ...
            [rx(:,rx_idx1),~,~] = func_awgn(rx(:,rx_idx1),snr,'measured');
        end
        ...
        for rx_idx4 = 1:numMultiAntennas
            ...
            tmp_estH = ones(4,numTags+numAddTags);
        ...
        for rx_idx7 = 1:numMultiAntennas
            payload_ofdmDemodPilots{rx_idx7} = ofdmDemodPilots{rx_idx7}(:,1+seqLenForEstChannel*(numTags+numAddTags)+1:end-numTail-numSymForTailPad);
        end
        payload_pilots = pilots(:,1+seqLenForEstChannel*(numTags+numAddTags)+1:end-numTail-numSymForTailPad);
        demodPayloadBits = survey_ParaFi_funcMultiAntennaDecoder(estH,payload_pilots,payload_ofdmDemodPilots,numTags+numAddTags);
