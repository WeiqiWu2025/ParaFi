...
%     disp(['SNR: ',num2str(snr(i)),' dB...']);
    ...
    while n<=maxNumPackets
        ...
        %%%%%** multi-antennas Tags-RX backscatter channel & AWGN & delay **%%%%%
        for chan_tag_rx_idx1 = 1:numMultiAntennas
            ...
            rxSig{chan_tag_rx_idx1,2} = [tmp_zeros;rxSig{chan_tag_rx_idx1,2}];
        end
       

        %%%%%*** RX side ***%%%%%
        rx = complex(zeros(length(rxSig{1,2}),numMultiAntennas));
        for rx_idx1 = 1:numMultiAntennas
            for rx_idx2 = 1:numTags
                [ttmp_l,ttmp_r] = size(rxSig{rx_idx1,rx_idx2}');
                rx(ttmp_l:ttmp_r,rx_idx1) = rx(ttmp_l:ttmp_r,rx_idx1) + rxSig{rx_idx1,rx_idx2};
            end
